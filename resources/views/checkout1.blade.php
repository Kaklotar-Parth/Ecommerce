                ...
                <div class="col-md-12 text-center services">
                    <div class="col-md-12 order-div">
                        <div class="col-md-7 order-left">
                            ...
                            <form class="col-md-offset-2 col-md-8" action="{{ route('payment.submit') }}" method="post"
                                id="payment_form">
                                {{ csrf_field() }}
                                ...
                                @else
                                    <div class="form-group">
                                        <input type="text" class="form-control" name="name"
                                            value="{{ Auth::guard('profile')->user()->name }}" placeholder="Full Name"
                                            required>
                                    </div>
                                    <div class="form-group">
                                        <input type="text" class="form-control" name="phone"
                                            value="{{ Auth::guard('profile')->user()->phone }}" placeholder="Phone Number"
                                            required>
                                    </div>
                                    <div class="form-group">
                                        <input type="email" class="form-control" name="email"
                                            value="{{ Auth::guard('profile')->user()->email }}" placeholder="Email"
                                            required>
                                    </div>
                                    <div class="form-group">
                                        <input type="text" class="form-control" name="address"
                                            value="{{ Auth::guard('profile')->user()->address }}" placeholder="Address"
                                            required>
                                    </div>
                                    <div class="form-group">
                                        <input type="text" class="form-control" name="city"
                                            value="{{ Auth::guard('profile')->user()->city }}" placeholder="City" required>
                                    </div>
                                    <div class="form-group">
                                        <input type="text" class="form-control" name="zip"
                                            value="{{ Auth::guard('profile')->user()->zip }}" placeholder="Postal Code"
                                            required>
                                    </div>
                        ...
                        <div class="col-md-5 order-right">
                            <h4>VIRTUAL PRODUCT ORDER DETAILS</h4>
                            <div class="pricing-list">
                                <table class="table">
                                    ...
                                    <tbody>
                                        @foreach ($cartdata as $cart)
                                            <tr>
                                                <td><a href="{{ url('/product') }}/{{ $cart->product }}/{{ str_replace(' ', '-', strtolower(\App\Product::findOrFail($cart->product)->title)) }}"
                                                        target="_blank">{{ $cart->title }}</a></td>
                                                <td>{{ $cart->quantity }}</td>
                                            </tr>
                                        @endforeach
                                    </tbody>
                                ...
                                <table class="table">
                                    ...
                                    <tr>
                                        <td>
                                            <h3 class="pricing-count" style="margin: 0">Total Cost:</h3>
                                        </td>
                                        <td width="30%">
                                            <h3 class="pricing-count" style="margin: 0">${{ round($total, 2) }}</h3>
                                        </td>
                                    </tr>
                                </table>
                            </div>
...
@section('footer')
    <script type="text/javascript">
        function meThods(val) {
            ...
            var action3 = "{{ route('cash1.submit') }}";
            if (val.value == "Paypal") {
