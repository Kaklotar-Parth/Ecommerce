                    ...
                    <table class="cart-table">
                        ...
                        @forelse($carts as $cart)
                            <tr id="item{{ $cart->product }}">
                                <td>
                                    <div class="traditional-cart-entry">
                                        <a href="#" class="image"><img
                                                src="{{ url('/assets/images/products') }}/{{ \App\Product::findOrFail($cart->product)->feature_image }}"
                                                alt=""></a>
                                        <div class="content">
                                            <div class="cell-view">
                                ...
                                <td id="price{{ $cart->product }}" class="prices">
                                    ${{ \App\Product::findOrFail($cart->product)->price }}</td>
                                <td>
                                    <div class="quantity-selector detail-info-entry">
                                        <div class="entry number-minus" id="minus{{ $cart->product }}">&nbsp;</div>
                                        <div class="entry number" id="number{{ $cart->product }}">{{ $cart->quantity }}
                                        </div>
                                        <div class="entry number-plus" id="plus{{ $cart->product }}">&nbsp;</div>
                                    </div>
                                </td>
                                <td>
                                    <div class="subtotal" id="subtotal{{ $cart->product }}">{{ $cart->cost }}</div>
                                </td>
                                <td><a class="remove-button" onclick="getDelete({{ $cart->product }})"><i
                                ...
                                <form id="citem{{ $cart->product }}">
                                    ...
                                    <input type="hidden" id="cost{{ $cart->product }}" name="cost"
                                        value="{{ $cart->cost }}">
                                    <input type="hidden" id="quantity{{ $cart->product }}" name="quantity"
                                        value="{{ $cart->quantity }}">
                                </form>
                        ...
                        <div class="cart-summary-box pull-right col-md-6" style="margin: 0">
                            ...
                            @php
                                ...
                                foreach ($carts as $cart) {
                                    $product = \App\Product::findOrFail($cart->product);
                            ...
                            @if ($hasVirtualProduct)
                                <!-- Show Virtual Checkout button and hide regular checkout -->
                                <a class="col-md-6 pull-right button style-10" href="{{ route('virtual.checkout1') }}">
                                    Proceed To Checkout
                                </a>
                            @else
                                <!-- Show Regular Checkout button only if no virtual products -->
                                <a class="col-md-6 pull-right button style-10" href="{{ route('user.checkout') }}">
                                    Proceed To Checkout
                                </a>
                            @endif
