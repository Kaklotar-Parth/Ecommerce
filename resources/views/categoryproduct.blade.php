...
@section('content')
    ...
    <section
        style="background: url({{ url('/') }}/assets/images/{{ $settings[0]->background }}) no-repeat center center; background-size: cover;">
                                            ...
                                            @foreach (\App\Category::where('mainid', $menu->id)->where('role', 'sub')->get() as $submenu)
                                                <li class="item-2 deeper parent">
                                                    <a class="" href="{{ url('/category') }}/{{ $submenu->slug }}">
                                                        <span data-toggle="collapse" data-parent="#menu-group-1"
                                                            href="#{{ $submenu->slug }}-1" class="sign"><i
                                                                class="fa fa-plus"></i></span>
                    ...
                    <div class="col-md-9">
                        ...
                        @if (count($products) > 0)
                            <div class="col-sm-12 col-xs-12 col-md-12 text-center" style="margin-top: 15px;">
                                ...
                                <div class="col-md-12">
                                    <img id="load" src="{{ url('/assets/images') }}/default.gif"
...
@section('footer')
    <script>
        ...
        $("#load-more").click(function() {
            ...
            var page = $("#page").val();
            var sort = $("#sortby").val();
            $.get("{{ url('/') }}/loadcategory/" + slug + "/" + page + "?sort=" + sort, function(data,
                status) {
                $("#load").fadeOut();
