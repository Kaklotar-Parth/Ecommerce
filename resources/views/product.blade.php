    ...
    <section class="container" style="margin-top: 20px;">
        <div class="content-push">
            ...
            <div class="breadcrumb-box">
                <a href="{{ url('/') }}">Home</a>
                @if (isset($productdata->category[0]))
                    <a
                        href="{{ url('/category') }}/{{ \App\Category::where('id', $productdata->category[0])->first()->slug }}">{{ \App\Category::where('id', $productdata->category[0])->first()->name }}</a>
                @endif
                @if (isset($productdata->category[1]) && $productdata->category[1] != '')
                    <a
                        href="{{ url('/category') }}/{{ \App\Category::where('id', $productdata->category[1])->first()->slug }}">{{ \App\Category::where('id', $productdata->category[1])->first()->name }}</a>
                @endif
                @if (isset($productdata->category[2]) && $productdata->category[2] != '')
                    <a
                        href="{{ url('/category') }}/{{ \App\Category::where('id', $productdata->category[2])->first()->slug }}">{{ \App\Category::where('id', $productdata->category[2])->first()->name }}</a>
                @endif
                <a
                    href="{{ url('/product') }}/{{ $productdata->id }}/{{ str_replace(' ', '-', strtolower($productdata->title)) }}">{{ $productdata->title }}</a>
            </div>
                                ...
                                <div class="swiper-wrapper">
                                    <div class="swiper-slide">
                                        <div class="product-zoom-image">
                                            @php $fileExtension = pathinfo($productdata->feature_image, PATHINFO_EXTENSION); @endphp
                                            @php $filePath = url('/assets/images/products/' . $productdata->feature_image); @endphp
                                            @if (in_array($fileExtension, ['jpg', 'jpeg', 'png', 'gif']))
                                                {{-- Show Image --}}
                                                <a
                                                    href="{{ url('/product') }}/{{ $productdata->id }}/{{ str_replace(' ', '-', strtolower($productdata->title)) }}">
                                            ...
                                            @elseif (in_array($fileExtension, ['mp4', 'avi', 'mov', 'webm']))
                                                ...
                                                <iframe src="{{ $filePath }}" width="100%" height="320px"
                                ...
                                <div class="swiper-container product-thumbnails-swiper" data-autoplay="0" data-loop="0"
                                    ...
                                    <div class="swiper-wrapper">
                                        ...
                                        @forelse($gallery as $galdta)
                                            <div class="swiper-slide">
                                                <div class="paddings-container">
                                                    <img src="{{ url('/') }}/assets/images/gallery/{{ $galdta->image }}"
                                                        alt="" />
                                                </div>
                                            </div>
                                        @empty
                        ...
                        <div class="product-detail-box">
                            ...
                            <div class="rating-box">
                                ...
                                <div class="rating-number">
                                    {{ \App\Review::where('productid', $productdata->id)->count() }}
                                    Reviews</div>
                            </div>
                            <div class="product-description detail-info-entry">
                                {{ substr(strip_tags($productdata->description), 0, 600) }}... <a id="showmore">Show
                                    More</a></div>
                            ...
                            <div class="detail-info-entry">
                                <form id="cartfrom">
                                    ...
                                    @else
                                        <button type="button" class="button style-10 to-cart" disabled>Out Of
                                            Stock</button>
                                    @endif
                                    {{-- <button type="button" class="button style-10 to-cart">Add to cart</button> --}}
                                </form>
            ...
            <div class="information-blocks">
                <div class="card">
                    ...
                    <ul class="nav nav-tabs" role="tablist">
                        <li role="presentation" class="active"><a href="#description" aria-controls="home"
                                role="tab" data-toggle="tab">Full Description</a></li>
                        <li role="presentation"><a href="#policy" aria-controls="profile" role="tab"
                                data-toggle="tab"> Return & Policy</a></li>
                        <li role="presentation"><a href="#reviews" aria-controls="messages" role="tab"
                                data-toggle="tab">Reviews({{ \App\Review::where('productid', $productdata->id)->count() }})</a>
                        </li>
                    </ul>
                                    ...
                                    <div class="form-group">
                                        ...
                                        <div class="col-md-4 col-md-offset-2">
                                            <button type="submit" class="button style-10"
                                                id="LoginButton"><strong>Submit Review</strong></button>
                                        </div>
                                    </div>
                                </div>
                            </form>
                            ...
                            <div class="photo">
                                <a
                                    ...
                                    <img src="{{ url('/assets/images/products') }}/{{ $product->feature_image }}"
                                        class="img-responsive" style="height: 320px;" alt="Product Image" /> </a>
                            </div>
                            <div class="info">
                                <div class="row">
                                    <div class="price-details">
                                        ...
                                        <a href="{{ url('/product') }}/{{ $product->id }}/{{ str_replace(' ', '-', strtolower($product->title)) }}"
                                            class="row" style="min-height: 60px">
                                            <h1>{{ $product->title }}</h1>
                                        </a>
                                        <div class="pull-left">
                                    ...
                                    <form>
                                        <p>
                                            ...
                                            @if ($product->stock != 0 || $product->stock === null)
                                                <button type="button" class="button style-10 to-cart">Add to
                                                    cart</button>
                                            @else
                                                <button type="button" class="button style-10 to-cart" disabled>Out Of
                                                    Stock</button>
